...
class Parser{
  ...
  Future<List<Map<String, String>>> fetchNewsList() async {
    ...
    for(Element el in elements){
      ...
      String newsUrl = siteUrl + el.querySelector('h3')!.querySelector('a')!.attributes['href']!;
      String imgUrl = siteUrl + el.querySelector('div.events-card__image img')!.attributes['src']!;
      news.add({
        'title': title,
        'url': newsUrl,
        'img_url': imgUrl
  ...
  Future<Map<String, dynamic>> fetchOneNews(String newsUrl) async{
  ...
  Future<String> fetchHowToEnroll(String url) async{
  ...
  Future<String> fetchContacts(String url) async{
    final response = await http.get(Uri.parse(url));
    final document = parser.parse(response.body);
